...
export function DashboardPage() {
  ...
  const [salesStatsByType, setSalesStatsByType] = useState<SalesStatsByType | null>(null);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [totalSalesData, statsData, statsByTypeData, departuresData] = await Promise.all([
  ...
  const getActiveBookings = () => {
    ...
    return (salesStats.salesByStatus.confirmed || 0) + (salesStats.salesByStatus.completed || 0);
  };
    ...
    <div className="space-y-6">
        ...
        <Card className="hover:shadow-lg transition-shadow border-l-4 border-l-blue-500">
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="text-2xl font-bold text-blue-600">{formatCurrency(totalSales, 'USD')}</div>
            )}
            <p className="text-xs text-gray-500 flex items-center mt-1">
              <TrendingUp className="mr-1 h-3 w-3 text-green-500" />
              {isAdmin ? 'Ingresos totales' : 'Mis ventas totales'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="hover:shadow-lg transition-shadow border-l-4 border-l-green-500">
          ...
          <CardContent>
            {loading ? (
              ...
              <>
                <div className="text-2xl font-bold text-green-600">{getActiveBookings()}</div>
                <p className="text-xs text-gray-500 mt-1">
                  {salesStats?.salesByStatus.confirmed || 0} confirmadas, {salesStats?.salesByStatus.completed || 0} completadas
                </p>
              </>
            )}
          </CardContent>
        ...
        <Card className="hover:shadow-lg transition-shadow border-l-4 border-l-purple-500">
          ...
          <CardContent>
            {loading ? (
              ...
              <>
                <div className="text-2xl font-bold text-purple-600">{salesStats?.salesCount || 0}</div>
                <p className="text-xs text-gray-500 mt-1">
        ...
        <Card className="hover:shadow-lg transition-shadow border-l-4 border-l-orange-500">
          ...
          <CardContent>
            {loading ? (
              ...
              <>
                <div className="text-2xl font-bold text-orange-600">{salesStats?.salesByStatus.draft || 0}</div>
                <p className="text-xs text-gray-500 mt-1">
                  {isAdmin ? 'Ventas en borrador esperando confirmación' : 'Mis ventas en borrador'}
                </p>
              </>
            )}
          </CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-white rounded-lg">
                ...
                <p className="text-sm font-medium text-gray-700">Gestión de Usuarios</p>
                <p className="text-xs text-gray-500">Administrar roles y permisos</p>
              </div>
              <div className="text-center p-4 bg-white rounded-lg">
                <PieChart className="h-8 w-8 text-green-600 mx-auto mb-2" />
                <p className="text-sm font-medium text-gray-700">Reportes Avanzados</p>
                <p className="text-xs text-gray-500">Análisis detallado de ventas</p>
              </div>
              <div className="text-center p-4 bg-white rounded-lg">
                <Activity className="h-8 w-8 text-purple-600 mx-auto mb-2" />
                <p className="text-sm font-medium text-gray-700">Métricas del Sistema</p>
                <p className="text-xs text-gray-500">Rendimiento y estadísticas</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-white rounded-lg">
                ...
                <p className="text-xs text-gray-500">Tus reservas confirmadas</p>
              </div>
              <div className="text-center p-4 bg-white rounded-lg">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-1 hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="space-y-4">
              {salesStats ? [
                { name: 'Borrador', count: salesStats.salesByStatus.draft || 0, status: 'draft', color: 'bg-gray-500' },
                { name: 'Confirmada', count: salesStats.salesByStatus.confirmed || 0, status: 'confirmed', color: 'bg-blue-500' },
                { name: 'Completada', count: salesStats.salesByStatus.completed || 0, status: 'completed', color: 'bg-green-500' },
                { name: 'Cancelada', count: salesStats.salesByStatus.cancelled || 0, status: 'cancelled', color: 'bg-red-500' },
              ].map((status, i) => (
                <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <div className={`h-8 w-8 rounded-full ${status.color} flex items-center justify-center`}>
                      {getStatusIcon(status.status)}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-900">{status.name}</p>
                      <p className="text-xs text-gray-500">{status.count} ventas</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-gray-900">{status.count}</p>
                    <p className="text-xs text-gray-500">
                      {salesStats.salesCount > 0 ? Math.round((status.count / salesStats.salesCount) * 100) : 0}%
                    </p>
                  </div>
                </div>
              )) : (
        ...
        <Card className="lg:col-span-1 hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="space-y-3">
              {upcomingDepartures.length > 0 ? upcomingDepartures.slice(0, 5).map((trip, i) => {
                ...
                return (
                  <div key={i} className="flex items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-900">{trip.passengerName}</p>
                      <p className="text-xs text-gray-500 capitalize">{trip.region} • {trip.serviceType}</p>
                    </div>
        ...
        <Card className="lg:col-span-1 hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="space-y-3">
              {salesStatsByType ? Object.entries(salesStatsByType.salesByServiceType)
                .sort(([,a], [,b]) => b.count - a.count)
                .slice(0, 5)
                .map(([type, data], i) => {
                  const totalCount = Object.values(salesStatsByType.salesByServiceType).reduce((sum, item) => sum + item.count, 0);
                  ...
                  return (
                    <div key={i} className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        <div className="font-medium">{getServiceTypeLabel(type)}</div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card className="hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="space-y-3">
              {salesStatsByType ? Object.entries(salesStatsByType.salesByRegion)
                .sort(([,a], [,b]) => b.count - a.count)
                .map(([region, data], i) => {
                  const totalCount = Object.values(salesStatsByType.salesByRegion).reduce((sum, item) => sum + item.count, 0);
                  const percentage = totalCount > 0 ? Math.round((data.count / totalCount) * 100) : 0;
                  const colors = ['bg-indigo-500', 'bg-pink-500', 'bg-yellow-500'];
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        <div className={`h-6 w-6 rounded-full ${colors[i % colors.length]} flex items-center justify-center`}>
                          <span className="text-xs text-white font-bold">{getRegionLabel(region).charAt(0).toUpperCase()}</span>
                        </div>
                        <div>
                          <p className="text-sm font-medium">{getRegionLabel(region)}</p>
                          <p className="text-xs text-gray-500">{data.count} ventas</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold">{percentage}%</p>
                        <p className="text-xs text-gray-500">{formatCurrency(data.totalCost, 'USD')}</p>
                      </div>
                    </div>
                  );
                }) : (
        ...
        <Card className="hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="space-y-3">
              {salesStatsByType ? Object.entries(salesStatsByType.salesBySaleType)
                .sort(([,a], [,b]) => b.count - a.count)
                .map(([type, data], i) => {
                  const totalCount = Object.values(salesStatsByType.salesBySaleType).reduce((sum, item) => sum + item.count, 0);
                  const percentage = totalCount > 0 ? Math.round((data.count / totalCount) * 100) : 0;
                  const colors = ['bg-teal-500', 'bg-cyan-500', 'bg-emerald-500', 'bg-sky-500'];
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        <div className={`h-6 w-6 rounded-full ${colors[i % colors.length]} flex items-center justify-center`}>
                          <span className="text-xs text-white font-bold">{getSaleTypeLabel(type).charAt(0).toUpperCase()}</span>
                        </div>
                        <div>
                          <p className="text-sm font-medium">{getSaleTypeLabel(type)}</p>
                          <p className="text-xs text-gray-500">{data.count} ventas</p>
                        </div>
                      </div>
                      <div className="text-right">
